...
> ## 📌 项目已于 2026-09-17 封存
>
> **回看这个项目，请先读 [`ARCHIVE.md`](ARCHIVE.md)。** 那里有：
> 一分钟版结论、12 项实验的索引（含限定条件与数据位置）、
> **15 条撤回台账**（哪些旧结论是错的、错在哪）、目录地图、以及停在哪里。
>
> ⚠️ 本仓库里的旧报告中留有已被推翻的结论。**以 `ARCHIVE.md` 的撤回台账为准。**

本仓库自包含，收录三条研究线的全部产出：结构发现（`progressive_task_discovery/`）、
状态抽象（`discovery/`）、结构利用（`results/`、`docs/`），以及逐次审计记录（`research/`）。

下面是封存前 AUTO 那一组对照实验的原始说明，仍然有效。
...
| **要查某个实验的数据和结论** | [**`docs/EXPERIMENT_INDEX.md`**](docs/EXPERIMENT_INDEX.md) |
