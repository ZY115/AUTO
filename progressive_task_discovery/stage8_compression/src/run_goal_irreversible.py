...
if __name__=="__main__":
    ...
    per_map={}
    for r in rows: per_map.setdefault((r["arm"],r["map"]),[]).append(r)
    def mean(arm,ms,key="first90"):
        # A resampled block drawn twice must count twice. Testing membership in
        # the sample silently deduplicated it, which shrank every interval.
        v=[r[key] for m in ms for r in per_map[(arm,m)]]
