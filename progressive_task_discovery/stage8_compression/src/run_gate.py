...
def main():
    ...
    shutil.copy(ROOT / 'src/gate.cpp', out / 'gate_snapshot.cpp')
    manifest = {'source_sha256': hashlib.sha256((ROOT / 'src/gate.cpp').read_bytes()).hexdigest(),
                'binary_sha256': hashlib.sha256((ROOT / 'gate').read_bytes()).hexdigest(),
