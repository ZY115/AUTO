...
struct Config{std::string method="count_replay";int k=8,seed=0,budget=80000,every=1000,hfactor=20,bridge=0,branching=1;double epsilon=.05,gamma=.98,reward_ratio=2,cost=.01;};
...
struct Environment{
    ...
    explicit Environment(const Config&c):H(c.hfactor*c.k),k(c.k),decision(c.bridge+1),branching(c.branching){
        std::mt19937_64 rng(mix(c.seed+111));
        seq[0]={0,1};seq[1]=branching?std::vector<int>{1,0}:std::vector<int>{0,1};
        for(int d=2;d<k;d++){int event;do{event=2+rng()%3;}while(d>2&&event==seq[0].back());seq[0].push_back(event);seq[1].push_back(event);}
        reset(0);
    }
    ...
    bool valid(int g)const{return depth==0?(g==0||(branching&&g==1)):(g==seq[branch][depth]);}
    Observation step(int action){
        auto m=mv[s][action];s=m.s;t++;bool p=false;
        if(depth==0&&(m.event==0||(branching&&m.event==1))){branch=m.event;p=true;}
...
struct RawSample{int s,a,ns,event;bool p,win;double reward;int episode,time;};
struct Agent{
    Config c;Context ctx;std::map<uint64_t,int> ids;std::vector<Node> nodes;
    std::map<int,std::set<int>> binding_edges;
    std::vector<double> Q,G;std::vector<RawSample> buffer;std::vector<std::vector<std::pair<int,int>>> raw_progress_events{{}};int episode_id=0,episode_time=0;std::array<std::vector<int>,2> oracle_seq;
    ...
    explicit Agent(const Config&c,const Environment&env):c(c),Q(SLOTS*N*4,0),G(M*N*4,0),
      skills(c.method.find("skill")!=std::string::npos),hand(c.method=="hand_replay"||c.method=="oracle_state_replay"||c.method=="hand_skill"||c.method=="oracle_skill"),
    ...
    int feature(const Context&x)const{if(c.method=="position_replay")return 0;if(count||(hand&&!c.branching))return x.depth;if(last_event)return x.depth==0?0:(x.depth-1)*M+int(x.key%6);if(hand)return x.depth==0?0:(x.depth<=c.bridge+1?2*x.depth-1+x.cue:x.depth+c.bridge+1);return node(x);}
    ...
    void update(const Sample&x){double reward=x.win?c.reward_ratio*c.cost*(12*c.k-21):-c.cost;double target=reward+(x.win?0:c.gamma*best(x.nf,x.ns));double&v=Q[(x.f*N+x.s)*4+x.a];v+=.3*(target-v);updates++;}
    Sample parse(const RawSample&r)const{
        Context before,after;
        for(auto [time,event]:raw_progress_events[r.episode]){if(time<r.time)before.advance(event);if(time<=r.time)after.advance(event);else break;}
        return{feature(before),r.s,r.a,feature(after),r.ns,r.p,r.win};
    }
    void observe(int s,int action,const Observation&o,int step){
        episode_time++;if(o.progress)raw_progress_events[episode_id].push_back({episode_time,o.event});
        ...
        if(skills){for(int g=0;g<M;g++){int base=(g*N+o.s)*4;double b=*std::max_element(G.begin()+base,G.begin()+base+4);bool hit=o.event==g;
            double target=double(hit)-.01+(hit?0:c.gamma*b);double&v=G[(g*N+s)*4+action];v+=.3*(target-v);updates++;}}
        else{RawSample raw{s,action,o.s,o.event,o.progress,o.success,o.success?c.reward_ratio*c.cost*(12*c.k-21):-c.cost,episode_id,episode_time};
            if(buffer.size()<10000)buffer.push_back(raw);else buffer[(step-1)%10000]=raw;
            for(int j=0;j<M;j++)update(parse(buffer[mix(uint64_t(step)*M+j+mix(c.seed+333))%buffer.size()]));}
        if(o.success)solved[next.cue]=true;ctx=o.done?Context{}:next;
        if(o.done){episode_id++;episode_time=0;raw_progress_events.emplace_back();}
    }
};
...
void run(const Config&c){
    ...
    std::map<int,long long> state_visits;double base_return=0;
    ...
    for(int step=1;step<=c.budget;step++){
        ...
        auto o=e.step(act);base_return+=o.success?c.reward_ratio*c.cost*(12*c.k-21):-c.cost;hashadd(trace,s+N*(act+4*(int(o.progress)+2*a.ctx.depth)));
    ...
    std::cout<<"{\"method\":\""<<c.method<<"\",\"k\":"<<c.k<<",\"seed\":"<<c.seed<<",\"bridge\":"<<c.bridge<<",\"branching\":"<<c.branching<<",\"budget\":"<<c.budget<<",\"horizon\":"<<e.H<<",\"epsilon\":"<<c.epsilon
      <<",\"gamma\":"<<c.gamma<<",\"reward_ratio\":"<<c.reward_ratio<<",\"terminal_reward\":"<<c.reward_ratio*c.cost*(12*c.k-21)<<",\"cost\":"<<c.cost<<",\"first90\":"<<hit<<",\"stable90\":"<<stable<<",\"auc\":"<<auc<<",\"final_success\":"<<last.e.mean()<<",\"final_branch_success\":";arr(last.e.success);
    ...
    std::cout<<"],\"feature_visit_counts\":[";bool first=true;double squares=0,entropy=0;for(auto [f,n]:state_visits){if(!first)std::cout<<",";first=false;std::cout<<"["<<f<<","<<n<<"]";double p=double(n)/c.budget;squares+=p*p;entropy-=p*std::log(p);}
    std::cout<<"],\"visited_features\":"<<state_visits.size()<<",\"effective_states_simpson\":"<<1/squares<<",\"effective_states_entropy\":"<<std::exp(entropy)<<",\"cumulative_base_reward\":"<<base_return<<",\"q_table_bytes\":"<<(a.Q.size()+a.G.size())*sizeof(double)<<",\"final_policy\":";arr(policy(a,e));
...
void selftest(){
    Config c;c.k=6;c.bridge=0;Environment e(c);
    assert(e.seq[0][0]==0&&e.seq[0][1]==1&&e.seq[1][0]==1&&e.seq[1][1]==0);
    for(int d=2;d<c.k;d++)assert(e.seq[0][d]==e.seq[1][d]);
    c.method="oracle_state_replay";Agent a(c,e);
    assert(a.feature(true_context(e,0,1))!=a.feature(true_context(e,1,1)));
    assert(a.feature(true_context(e,0,2))==a.feature(true_context(e,1,2)));
    assert(a.predicted_goal(Context{})==-1);
    std::cout<<"{\"unordered_self_tests\":\"passed\"}\n";
}
int main(int argc,char**argv){
    ...
    Config c;for(int i=1;i<argc;i+=2){if(i+1>=argc)throw std::runtime_error("missing argument");std::string f=argv[i],v=argv[i+1];
      if(f=="--gamma")c.gamma=std::stod(v);else if(f=="--reward-ratio")c.reward_ratio=std::stod(v);else if(f=="--method")c.method=v;else if(f=="--k")c.k=std::stoi(v);else if(f=="--seed")c.seed=std::stoi(v);else if(f=="--budget")c.budget=std::stoi(v);else if(f=="--every")c.every=std::stoi(v);else if(f=="--hfactor")c.hfactor=std::stoi(v);else if(f=="--bridge")c.bridge=std::stoi(v);else if(f=="--branching")c.branching=std::stoi(v);else if(f=="--epsilon")c.epsilon=std::stod(v);else throw std::runtime_error("unknown flag");}
    const std::set<std::string> methods={"count_replay","last_event_replay","history_replay","hand_replay","learned_replay","delayed_skill","immediate_skill","oracle_skill","history_skill","hand_skill","position_replay","oracle_state_replay"};
    if(!methods.count(c.method)||c.k<2||c.k>20||c.bridge!=0||c.k<c.bridge+2||c.budget<=0||c.every<=0||c.hfactor<12||c.epsilon<0||c.epsilon>1||(c.branching!=0&&c.branching!=1))throw std::runtime_error("invalid config");run(c);
}
