...
struct Agent{
    ...
    void update(const Sample&x){double reward=x.win?c.reward_ratio*c.cost*(12*c.k-11):-c.cost;double target=reward+(x.win?0:c.gamma*best(x.nf,x.ns));double&v=Q[(x.f*N+x.s)*4+x.a];v+=.3*(target-v);updates++;}
    ...
    void observe(int s,int action,const Observation&o,int step){
        ...
        else{RawSample raw{s,action,o.s,o.event,o.progress,o.success,o.success?c.reward_ratio*c.cost*(12*c.k-11):-c.cost,episode_id,episode_time};
...
void run(const Config&c){
    ...
    std::cout<<"{\"method\":\""<<c.method<<"\",\"k\":"<<c.k<<",\"seed\":"<<c.seed<<",\"bridge\":"<<c.bridge<<",\"branching\":"<<c.branching<<",\"budget\":"<<c.budget<<",\"horizon\":"<<e.H<<",\"epsilon\":"<<c.epsilon
      <<",\"gamma\":"<<c.gamma<<",\"reward_ratio\":"<<c.reward_ratio<<",\"terminal_reward\":"<<c.reward_ratio*c.cost*(12*c.k-11)<<",\"cost\":"<<c.cost<<",\"first90\":"<<hit<<",\"stable90\":"<<stable<<",\"auc\":"<<auc<<",\"final_success\":"<<last.e.mean()<<",\"final_branch_success\":";arr(last.e.success);
