...
struct Config{std::string method="count_replay";int k=8,seed=0,budget=80000,every=1000,hfactor=20,bridge=0;double epsilon=.05;};
...
struct Environment{
    ...
    int s,depth=0,branch=-1,t=0,H,k,decision;
    explicit Environment(const Config&c):H(c.hfactor*c.k),k(c.k),decision(c.bridge+1){
    ...
    int true_state()const{return depth==0?0:(depth<=decision?2*depth-1+branch:depth+decision);}
...
struct Agent{
    ...
    int feature(const Context&x)const{if(count)return x.depth;if(last_event)return x.depth==0?0:(x.depth-1)*M+int(x.key%6);if(hand)return x.depth==0?0:(x.depth<=c.bridge+1?2*x.depth-1+x.cue:x.depth+c.bridge+1);return node(x);}
...
Diagnostics diagnostics(const Agent&a,const Environment&e){
    ...
    for(int b=0;b<2;b++)for(int d=0;d<e.k;d++){
        auto x=true_context(e,b,d);int f=a.feature(x),q=d==0?0:(d<=e.decision?2*d-1+b:d+e.decision);truth.insert(q);
...
void run(const Config&c){
    ...
    std::cout<<"{\"method\":\""<<c.method<<"\",\"k\":"<<c.k<<",\"seed\":"<<c.seed<<",\"bridge\":"<<c.bridge<<",\"budget\":"<<c.budget<<",\"horizon\":"<<e.H<<",\"epsilon\":"<<c.epsilon
...
int main(int argc,char**argv){
    ...
    const std::set<std::string> methods={"count_replay","last_event_replay","history_replay","hand_replay","learned_replay","delayed_skill","immediate_skill","oracle_skill","history_skill"};
    if(!methods.count(c.method)||c.k<3||c.k>16||c.bridge<0||c.k<c.bridge+2||c.budget<=0||c.every<=0||c.hfactor<12||c.epsilon<0||c.epsilon>1)throw std::runtime_error("invalid config");run(c);
}
