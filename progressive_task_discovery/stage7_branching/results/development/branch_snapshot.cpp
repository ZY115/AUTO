...
constexpr int W=13,N=169,M=5,SLOTS=64;
...
struct Config{std::string method="count_replay";int k=8,seed=0,budget=80000,every=1000,hfactor=20;double epsilon=.05;};
...
struct Environment{
    ...
    int s,depth=0,branch=-1,t=0,H,k;
    explicit Environment(const Config&c):H(c.hfactor*c.k),k(c.k){
        std::mt19937_64 rng(mix(c.seed+111));std::vector<int> g={2,3,4};
        for(int i=2;i>0;i--)std::swap(g[i],g[rng()%(i+1)]);
        seq[0]={0,g[0]};seq[1]={1,g[1]};
        if(k>2){seq[0].push_back(g[2]);seq[1].push_back(g[2]);}
        for(int d=3;d<k;d++){int e;do{e=2+rng()%3;}while(e==seq[0].back());seq[0].push_back(e);seq[1].push_back(e);}
        reset(0);
    }
    void reset(int context){s=START[context];depth=0;branch=-1;t=0;}
    int true_state()const{return depth==0?0:(depth==1?branch+1:depth+1);}
...
struct Agent{
    ...
    bool skills,hand,count,oracle,delay;
    explicit Agent(const Config&c,const Environment&env):c(c),Q(SLOTS*N*4,0),G(M*N*4,0),
      skills(c.method.find("skill")!=std::string::npos),hand(c.method=="hand_replay"||c.method=="oracle_skill"),
      count(c.method=="count_replay"),oracle(c.method=="oracle_skill"),delay(c.method=="delayed_skill"){
    ...
    int feature(const Context&x)const{if(count)return x.depth;if(hand)return x.depth==0?0:(x.depth==1?x.cue+1:x.depth+1);return node(x);}
    ...
    void observe(int s,int action,const Observation&o,int step){
        ...
        if(o.progress){next.advance(o.event);int dest=ensure(next.key);if(nodes[oldnode].edges[o.event]<0){nodes[oldnode].edges[o.event]=dest;nodes[oldnode].discovered[o.event]=step;}}
...
Diagnostics diagnostics(const Agent&a,const Environment&e){
    ...
    for(int b=0;b<2;b++)for(int d=0;d<e.k;d++){
        auto x=true_context(e,b,d);int f=a.feature(x),q=d==0?0:(d==1?b+1:d+1);truth.insert(q);
        if(f>=0){counts[f][q]++;if(d==1)junction[f][q]++;}
...
void run(const Config&c){
    ...
    for(int step=1;step<=c.budget;step++){
        ...
        if(g>=0){bindings++;if(!e.valid(g))wrong++;if(segment_goal<0||segment_key!=a.ctx.key){skill_attempts++;segment_goal=g;segment_key=a.ctx.key;}}
        ...
        if(segment_goal>=0&&(o.progress||o.done)){if(o.event==segment_goal&&o.progress)skill_success++;else if(o.progress)skill_other_progress++;segment_goal=-1;}
    ...
    std::cout<<"{\"method\":\""<<c.method<<"\",\"k\":"<<c.k<<",\"seed\":"<<c.seed<<",\"budget\":"<<c.budget<<",\"horizon\":"<<e.H<<",\"epsilon\":"<<c.epsilon
    ...
    if(a.count)std::cout<<",\"stochastic_final_success_e005\":"<<stochastic_count_eval(a,e,.05)<<",\"stochastic_final_success_e02\":"<<stochastic_count_eval(a,e,.2);
    std::cout<<"}\n";
}
void selftest(){
...
int main(int argc,char**argv){
    ...
    Config c;for(int i=1;i<argc;i+=2){if(i+1>=argc)throw std::runtime_error("missing argument");std::string f=argv[i],v=argv[i+1];
      if(f=="--method")c.method=v;else if(f=="--k")c.k=std::stoi(v);else if(f=="--seed")c.seed=std::stoi(v);else if(f=="--budget")c.budget=std::stoi(v);else if(f=="--every")c.every=std::stoi(v);else if(f=="--hfactor")c.hfactor=std::stoi(v);else if(f=="--epsilon")c.epsilon=std::stod(v);else throw std::runtime_error("unknown flag");}
    const std::set<std::string> methods={"count_replay","history_replay","hand_replay","learned_replay","delayed_skill","immediate_skill","oracle_skill","history_skill"};
    if(!methods.count(c.method)||c.k<3||c.k>16||c.budget<=0||c.every<=0||c.hfactor<12||c.epsilon<0||c.epsilon>1)throw std::runtime_error("invalid config");run(c);
}
