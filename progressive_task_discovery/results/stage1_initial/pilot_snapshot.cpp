...
struct Agent {
    ...
    int stage=0,depth=0;
    long long updates=0;
    bool flat,adaptive,cf,oracle;
    explicit Agent(const Config &config):c(config),Q(MAXQ*N*4,0),flat(c.method=="flat"),
        adaptive(c.method=="progressive"||c.method=="count_frontier"||c.method=="progressive_qrm"),
        cf(c.method=="learned_qrm"||c.method=="progressive_qrm"||c.method=="oracle_qrm"),oracle(c.method=="oracle_qrm") {edge.fill(-1);}
    ...
    int greedy(int q,int s)const {int a=0;for(int b=1;b<4;b++)if(Q[ix(q,s,b)]>Q[ix(q,s,a)])a=b;return a;}
    ...
    int act(int s,double er,uint64_t ra,uint64_t tie)const {
        ...
        double v=best(stage,s);int actions[4],n=0;
        for(int a=0;a<4;a++)if(Q[ix(stage,s,a)]==v)actions[n++]=a;
    ...
    void one_update(int q,int s,int a,int ns,bool progress,bool success) {
        ...
        double target=reward+(success?0:c.gamma*best(nq,ns));
...
void run(const Config &c) {
    ...
    for(int step=1;step<=c.budget;step++) {
        ...
        if(o.done){episodes++;if(o.success){train_wins++;if(first_train_success<0)first_train_success=step;}env.reset();}
    ...
    std::cout<<"{\"method\":\""<<c.method<<"\",\"seed\":"<<c.seed<<",\"k\":"<<c.k<<",\"budget\":"<<c.budget<<",\"horizon\":"<<env.H<<",\"slip\":"<<c.slip
       <<",\"epsilon\":"<<c.eps<<",\"high\":"<<c.high<<",\"low\":"<<c.low<<",\"master_n\":"<<c.master_n<<",\"eta\":"<<c.eta
...
int main(int argc,char**argv) {
    ...
    for(int i=1;i<argc;i+=2){if(i+1>=argc)throw std::runtime_error("missing value");std::string f=argv[i],v=argv[i+1];
        ...
        else if(f=="--master-n")c.master_n=std::stoi(v);else if(f=="--eta")c.eta=std::stod(v);else throw std::runtime_error("unknown flag "+f);
    }
    const std::vector<std::string> methods={"flat","count","discovered","oracle_state","progressive","count_frontier","learned_qrm","progressive_qrm","oracle_qrm"};
