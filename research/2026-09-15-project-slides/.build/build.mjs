...
// 01
{
 const s=slide('未知任务中的在线发现',notes('research/2026-09-15-memory-compression-audit/PROJECT_STATUS.md','问题是隐藏任务顺序/逻辑的发现，不是完全没有任务信号的目标发现。事件可识别，早期实验提供可靠的逐步推进反馈。格子图仅为机制示意，不对应具体实验地图。'));
...
// 02
{
 const s=slide('线性任务：立即使用已发现的前缀',notes('progressive_task_discovery/REPORT.md','Stage 6：长度16，动作噪声，第二批32个新种子。左图为首次成功率达到90%的平均环境步。右图为第一次完整成功之前，在已知目标前缀中累计执行的平均步数，两者观察区间不同，不应当相减或当成总成本的精确分解。三方法每步9次Bellman更新；立即/延迟epsilon均为0.005，普通replay为0.05（开发集选择）。未声称超过ISA等既有方法。'),'长度 16，32 个种子；立即与延迟是同配置的时机对照，普通 replay 探索率不同，仅作参考基线');
 text(s,'首次达到 90%',80,137,620,38,27);text(s,'首次完整成功前：已知前缀',816,137,400,38,25);
 chart(s,'bar',['普通 replay','延迟使用','立即使用'],[{name:'平均 first90',values:[18594,37938,6906],fill:C.blue,points:[{idx:0,fill:C.gray},{idx:1,fill:C.orange},{idx:2,fill:C.blue}]}],65,198,705,413,{yAxis:{min:0,max:45000,majorUnit:10000,numberFormatCode:'#,##0',textStyle:{fontSize:18,fill:C.muted},majorGridlines:{fill:'#E9EEF1',width:1}},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:27,fill:C.ink}}});
 chart(s,'bar',['延迟','立即'],[{name:'已知前缀执行步',values:[33194,2566],fill:C.blue,points:[{idx:0,fill:C.orange}]}],812,198,404,413,{yAxis:{min:0,max:40000,majorUnit:10000,numberFormatCode:'#,##0',textStyle:{fontSize:16,fill:C.muted},majorGridlines:{fill:'#E9EEF1',width:1}},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:25,fill:C.ink}}});
}
// 03
{
 const s=slide('线性任务的最小记忆',notes('research/2026-09-15-memory-compression-audit/PROJECT_STATUS.md','在线性任务与可靠进度反馈条件下，进度唯一标记任务阶段。列表是链式自动机的等价表示；结果不证明图形式本身的优势。未知项由试错逐步填入。'));
 text(s,'阶段列表',148,146,400,42,28);text(s,'链式自动机',721,146,400,42,28);
 text(s,'progress',125,222,170,36,24,C.muted);text(s,'goal',345,222,160,36,24,C.muted);
 [ ['0','A'],['1','B'],['2','C'],['3','?']].forEach(([a,b],i)=>{let y=282+i*70;line(s,116,y+56,385);text(s,a,155,y,90,50,32);text(s,b,357,y,90,50,32,i===3?C.muted:C.blue);});
 text(s,'≡',572,356,70,85,62,C.muted,false,'center');
 const arr=[node(s,'q₀',730,262,95,68),node(s,'q₁',1020,262,95,68),node(s,'q₂',1020,466,95,68),node(s,'q₃',730,466,95,68,C.muted)];
 conn(s,arr[0],arr[1]);conn(s,arr[1],arr[2],C.gray,'bottom','top');conn(s,arr[2],arr[3],C.gray,'left','right');
 text(s,'A',885,250,55,50,30,C.blue);text(s,'B',1110,368,50,50,30,C.blue);text(s,'C',885,510,55,50,30,C.blue);
}
// 04
{
 const s=slide('分支任务：相同进度，不同未来',notes('progressive_task_discovery/stage7_branching/REPORT.md','A→E与B→E汇入同一物理位置，进度均为2、末事件均为E，但后继要求不同。图中两个E表示不同历史下的任务状态，不是两个物理位置。充分历史可以解决这个任务。咖啡例子只是同构的直觉说明，不是实验中的任务：A与B是两个不同的首步（烧开水 / 取冷水），E是两边共有的同一个动作（冲入咖啡粉）。'),'两条历史进度相同（都做了 2 步）、刚完成的动作也相同（都是冲粉），但下一步完全不同 —— 只看进度和当前位置分不出来');
 text(s,'已发生的历史',105,151,400,40,25,C.muted);text(s,'相同物理位置 E',529,151,330,40,25,C.muted);text(s,'下一目标',994,151,210,40,25,C.muted);
 const a=node(s,'A',150,267),b=node(s,'B',150,458,96,68,C.orange),ea=node(s,'E',621,267),eb=node(s,'E',621,458,96,68,C.orange),d=node(s,'D',1040,267),c=node(s,'C',1040,458,96,68,C.orange);
 conn(s,a,ea,C.blue);conn(s,b,eb,C.orange);conn(s,ea,d,C.blue);conn(s,eb,c,C.orange);
 text(s,'progress = 2',525,373,287,45,29,C.ink,false,'center');
 text(s,'A, E',305,235,200,45,24,C.blue,false,'center');text(s,'B, E',305,426,200,45,24,C.orange,false,'center');
 text(s,'先烧开水',122,345,152,34,21,C.blue,false,'center');
 text(s,'冲入咖啡粉',581,345,176,34,21,C.muted,false,'center');
 text(s,'3 分钟后过滤 = 热泡',965,345,246,34,21,C.blue,false,'center');
 text(s,'先取冷水',122,536,152,34,21,C.orange,false,'center');
 text(s,'冲入咖啡粉',581,536,176,34,21,C.muted,false,'center');
 text(s,'冷藏 12 小时 = 冷萃',965,536,246,34,21,C.orange,false,'center');
 text(s,'同一个「冲粉」动作，之前不同，之后就完全不同',432,600,472,38,23,C.ink,true,'center');
}
// 05
{
 const s=slide('同一份历史的两种表示',notes('progressive_task_discovery/stage7_branching/REPORT.md','前缀树与完整成功事件历史dictionary在匹配输入与随机数时逐动作一致。字典包含足够历史，并非只存进度或最后事件。这里比较表示形式，不涉及合并不同历史。'));
...
// 06
{
 const s=slide('状态抽象：合并未来等价的历史',notes('AUTO/docs/MEMORY_COMPRESSION_PILOT.md','零件任务规则：A先于B则选X，否则选Y，其他零件位置无关。完整顺序由环境提供；末端只做一次二选一。此图表示末端决策等价，不声称每个中间前缀在环境状态上都等价。正确自动机与等价手写程序使用同一状态划分。'),'咖啡：磨豆、称粉、拿杯子的先后都不影响结果 —— 这些历史可以合并；只有决定热泡还是冷萃的那一步必须单独记 —— 这些不能合并');
...
// 07
{
 const s=slide('正确压缩与逐条历史学习',notes('AUTO/results/runs/memory_compression_pilot_5k.md','每个规模400个配对种子，每方法5000幕。T95为首次达到并在后续记录点保持至少95%准确率的幕数，本批主要对照无删失，因此RMST等于平均T95。压缩等价关系由设计者提供。完整历史与正确压缩使用同样Q-learning。自动机与等价手写程序逐种子逐评估点一致。比值来自未取整均值，不应以显示取整值要求精确相除。该量级有闭式解释：错误动作Q恒为0，正确动作首次获正奖励后Q=1，此后该状态永远答对；未覆盖状态随机并列答对一半，故准确率=(H+c)/2H，要达95%需覆盖c>=0.9H。已覆盖i格时新覆盖一格的概率为(H-i)/(2H)，期望等待2H/(H-i)，累加得 sum(2H/(H-i), i=0..ceil(0.9H)-1)。H=6/24/120给出29.40/109.25/543.76幕，压缩后H=2给出6.0幕，理论倍数90.63x，实测89.75x。大倍数来自填表次数，不需要额外机制假设。'),'正确等价关系已知；每规模 400 个配对种子（不是 400 个任务：候选族只有 6 / 12 / 20 条规则）');
 ...
 text(s,'闭式解 Σ 2H/(H-i)，H = n! = 6 / 24 / 120 → 理论 29.4 / 109.2 / 543.8 幕，与观测一致',80,616,1120,34,19,C.muted,false,'center');
}
// 08
{
 const s=slide('有限证据下的合并判断',notes('progressive_task_discovery/stage8_compression/REPORT_STEP35.md','机制示意。左侧：暂时具有相同观测的历史可能在未见后缀上要求不同操作；合并会错用经验。右侧：等价历史始终分开会重复学习。示意不是本次审计的原始自动机，不把当前裁决相同视为全后缀等价。'),'咖啡：两条历史现在都「刚冲完粉」，但一条 3 分钟后要过滤、一条要冷藏 12 小时 —— 现在合并，就是把冷萃的步骤用到热泡上');
 text(s,'过早合并',180,135,390,45,29,C.red);text(s,'保守不合并',814,135,380,45,29,C.ink);
 const a=node(s,'历史 A',85,270,140,65,C.gray),b=node(s,'历史 B',85,463,140,65,C.gray),q=node(s,'同一状态',347,365,155,70,C.red);conn(s,a,q,C.red);conn(s,b,q,C.red);
 text(s,'过滤？',425,240,170,45,27,C.red);text(s,'冷藏？',425,506,170,45,27,C.red);text(s,'未见后缀可能不同',128,588,420,40,24,C.red,false,'center');
 ['ABC','ACB','CAB'].forEach((t,i)=>{let yy=242+i*122;let h=node(s,t,743,yy,126,66,C.gray),z=node(s,'Q(X/Y)',1025,yy,160,66,C.blue);conn(s,h,z);});
 text(s,'等价历史分别学习',757,588,420,40,24,C.muted,false,'center');
}
// 09
{
 const s=slide('合并审计：正确性与查询节省',notes('progressive_task_discovery/stage8_compression/REPORT_STEP35.md','Step35：n=3的8个任务族，792次查询。固定基线轨迹，逐查询只使用当时证据进行回放审计；未将这些合并接入控制器，也没有相应学习曲线。k_min是当时证据所容许的最小机器状态数，+1放松该上界。节省只计审计识别的可安全节省部分（134/792），那102次落在必要查询上的复用不计入节省，它是另一种危害：把真正需要买的答案用别人的答案替掉。判据在跑之前写定：节省>=20%且错误共享约等于0。在线k_min是8-12而真状态数是19-27，证据少时最小机器严重过度合并，这是离线97.6%精确率无法在线复现的全部原因。'),'查询节省 16.9% < 20%；错误裁决 47 次；错误归属 140 次；把 53 次求解超时全判给 Occam，节省上界 23.6% 仍不达标');
 ['裁决正确率','状态归属正确率','可省查询比例'].forEach((t,i)=>text(s,t,67+i*402,145,363,45,27,C.ink,false,'center'));
 [[.801,.773],[.407,.636],[.169,.018]].forEach((v,i)=>chart(s,'bar',['k_min','k_min+1'],[{name:'比例',values:v,valuesFormatCode:'0.0%',fill:C.blue,points:[{idx:1,fill:C.orange}]}],62+i*402,220,353,382,{yAxis:{min:0,max:1,majorUnit:.25,numberFormatCode:'0%',textStyle:{fontSize:17,fill:C.muted},majorGridlines:{fill:'#E9EEF1',width:1}},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:24,fill:C.ink}}}));
 text(s,'预注册判据 3 / 3 不达标 —— 这条线因此关闭',80,606,1120,40,26,C.red,true,'center');
}
// 10
{
 const s=slide('可靠任务反馈的预算',notes('AUTO/discovery/src/run_verify.py','原始归档AUTO/discovery/results/verify.jsonl.gz：预算40/80/160/320，first90<200000的运行分别1/61/131/144，共144。图中百分比按用户口播四舍五入为1/42/91/100。嵌入图表数据保留12位小数，分子分母在页面与备注中保留精确计数。160预算时最终仍>=90%的只有127/144；本图量的是曾经达标，不是最终成功比例。控制器按历史节点读取局部证据，未启用合并。不能由最终树节点数推断必须完整结构才能使用。'),'24 个任务族 × 6 个种子；200,000 环境步内曾达到 90%');
 chart(s,'bar',['40','80','160','320'],[{name:'达标运行比例',values:[0.006944444444,0.423611111111,0.909722222222,1],valuesFormatCode:'0%',fill:C.blue}],105,151,1080,430,{yAxis:{min:0,max:1.12,majorUnit:.25,numberFormatCode:'0%',textStyle:{fontSize:19,fill:C.muted},majorGridlines:{fill:'#E9EEF1',width:1}},xAxis:{title:'提问预算 / 次',textStyle:{fontSize:25,fill:C.ink},line:{fill:C.line,width:1},majorGridlines:null},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:30,fill:C.ink}}});
 ['1 / 144','61 / 144','131 / 144','144 / 144'].forEach((t,i)=>text(s,t,209+i*245,590,190,40,23,C.muted,false,'center'));
}
// 11  (原13：提前使用)
{
 const s=slide('提前使用：主动探索与被动证据',notes('research/2026-09-15-memory-compression-audit/REVIEW.md','最新零件pilot每规模400种子。训练期间、规则唯一识别前立即法相比延迟法实际额外成功均值为0.1225、0.4525、0.635，图显示口播取整0.123/0.453/0.635。不是贪心评估累计期望0.137/0.533/0.696。规则识别前准确率不足95%；识别后两臂在该实现中相同，因此T95没有分辨能力。该必然性只针对这个规则族、确定反馈、更新与阈值设置。主动任务早期调用技能改变到达后缀的行为，被动任务每幕独立给出排列，动作不影响下一份证据。不能据此断言所有边学边用收益都必须依赖主动取证。'),'零件实验：识别前未达 95%，识别后策略相同，因此 T95 相同');
 text(s,'早期导航任务',100,140,410,40,26);text(s,'最新零件实验',752,140,425,40,26);
 ...
 let d=node(s,'环境给出排列',718,220,208,62,C.gray),e=node(s,'选择 X / Y',1011,220,188,62,C.blue);conn(s,d,e);text(s,'下一幕仍由环境独立抽样',756,325,430,55,23,C.muted,false,'center');
 text(s,'识别前实际额外成功 / 次',79,477,540,48,28);
 chart(s,'bar',['3 个','4 个','5 个'],[{name:'立即减延迟',values:[.123,.453,.635],valuesFormatCode:'0.000',fill:C.blue}],684,420,528,208,{yAxis:{min:0,max:.85,majorUnit:.4,numberFormatCode:'0.0',textStyle:{fontSize:16,fill:C.muted},majorGridlines:{fill:'#E9EEF1',width:1}},xAxis:{textStyle:{fontSize:20,fill:C.ink},majorGridlines:null},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:24,fill:C.ink}}});
 text(s,'每规模 400 个配对种子',80,533,520,38,23,C.muted);
}
// 12  (原11：路由与复用)
{
 const s=slide('结构利用：任务路由与技能复用',notes('progressive_task_discovery/stage8_compression/REPORT_STEP36.md','24族×30种子×5臂，预算200000步。first90是报告中的中位数，AUC为成功率曲线面积。Y00没有两种机制，Y01用学习型元控制器调用共享技能，Y10真规则路由但技能按任务状态独立，Y11真规则路由且跨状态共享技能；QRM单列。Y01并未被给定正确目标顺序，结构化臂用真规则而不是在线学规则。原报告关于路由压倒性强于复用的AUC解释已在Step38撤回，因天花板影响；不能用本页做该推断。后续C-full审计指出Y10/Y11差异不只来自参数共享，还涉及经验传播、采样和探索计数。删失：原始归档results/factorial.json中first90=-1（从未达标）共40次，全部落在Y00（30/720）与Y01（10/720），Y10/Y11/QRM为0。页面中位数按达标者计；Y00若把-1记作预算200000，中位为41000而非38500。该丢弃使Y00/Y01显得偏好，因此本页对比是保守的。另：报告曾给出样本复杂度主效应路由6.39x、复用5.34x，本次未能从归档复现（把-1记作预算得9.03x/4.50x），故本页不列该口径。'),'已知规则的控制消融；AUC 接近天花板，不能据此排序机制强弱');
...
// 13  (原12：危险格)
{
 const s=slide('危险格数量与达标率',notes('progressive_task_discovery/stage8_compression/REPORT_STEP38_5.md','Step38.5剂量扫描，昂贵导航，自有导出模拟器，任务规则已给定，训练预算1000000环境步。达标率为预算内达到90%成功率的运行比例。Y10/Y11数据按历史报告提供。此扫描的逐种子原始数据后来确认遗失，因此不能重算置信区间。Step40新网格逐运行归档：步费改变QRM排序，并使风险交互效应缩小；不能将此页视为协议不变的效应大小。无论存在何种图论路径，危险格可能迫使成功轨迹绕行。这里不声明几何难度完全不变。页底两行取自REPORT_STEP38_5第125-126行：昂贵地图加岩浆时「有技能无路由」0.882远胜「有路由无技能」0.106。'),'已知规则；固定步费协议；100 万步预算；该扫描逐种子记录已缺失，不能重算区间');
...
// 14
{
 const s=slide('三层研究关系',notes('research/2026-09-15-memory-compression-audit/PROJECT_STATUS.md','这三层不是全都已集成的一套通用算法。结构发现的受限未知顺序闭环有正结果；状态抽象的已知正确压缩有明确收益，可靠在线合并尚未解决；结构利用的路由/共享实现具有条件性收益，但一般机制归因、协议与任务边界未定。未建立超过ISA等的通用新方法，不声称自动机语法优于等价手写实现。'));
...
await fs.mkdir(WORK+'/.build/previews',{recursive:true});
await (await PresentationFile.exportPptx(p)).save(WORK+'/.build/candidate.pptx');
for(let i=0;i<p.slides.items.length;i++){
 const s=p.slides.items[i];const png=await p.export({slide:s,format:'png',scale:1});await fs.writeFile(WORK+'/.build/previews/'+String(i+1).padStart(2,'0')+'.png',new Uint8Array(await png.arrayBuffer()));
 const lay=await s.export({format:'layout'});await fs.writeFile(WORK+'/.build/previews/'+String(i+1).padStart(2,'0')+'.json',await lay.text());
 console.log('rendered',i+1);
}
const FINAL=WORK+'/output/任务结构研究_口播配套_v2.pptx';
const result=await finalizePresentation({workspaceDir:WORK,candidatePath:WORK+'/.build/candidate.pptx',finalPath:FINAL,
 ...
 receiptPath:WORK+'/.build/validation_v9.json'});
