...
int main(int argc,char**argv){
    ...
    long long audit_entries=0,audit_sink_steps=0,audit_sink_events=0,audit_sink_conflicts=0;long long failures=0,doomed_steps=0;double auc=0,last=0;
    for(int step=1;step<=c.budget;step++){
        ...
        bool fell=T.failing[nq]!=0; if(fell&&!T.failing[qstate])audit_entries++; if(T.failing[qstate])audit_sink_steps++;
        ...
        if(L.tracking&&ev>=0&&!progress){ if(T.failing[qstate]){audit_sink_events++;if(L.tree.advanced[node][ev]>0)audit_sink_conflicts++;}
    ...
    std::cerr<<"{\"failure_entries\":"<<audit_entries<<",\"post_failure_steps\":"<<audit_sink_steps<<",\"post_failure_events_written_to_healthy_node\":"<<audit_sink_events<<",\"contradictions_with_prior_progress\":"<<audit_sink_conflicts<<"}\n";
